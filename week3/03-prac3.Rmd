...
2.  Once we've downloaded the `.gpkg` let's see what is inside it with `st_layers()`...

```{r message=FALSE, cache=FALSE}
library(sf)
library(here)
st_layers(here("week3", "prac3_data", "gadm41_AUS.gpkg"))
```
...
```{r, cache=FALSE}
library(sf)
Ausoutline <- st_read(here("week3","prac3_data", "gadm41_AUS.gpkg"), 
                      layer='ADM_ADM_0')
```
...
library(raster)
library(terra)
jan<-terra::rast(here("week3", "prac3_data", "wc2.1_5m_tavg", "wc2.1_5m_tavg_01.tif"))
...
2.  But let's firstly explore what `dir_info()` does...
...
You could also run this using the original worldclimtemp raster, however, it may take some time. I'd recommend cropping to the extent first. 1. Let's re-compute our histogram for Australia in March. We could just use hist like we have done before. We can either subset using the location (we know March is third in the raster).
